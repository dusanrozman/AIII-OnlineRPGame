...
struct cname cname[]={
	...
	{"Sports","Discuss sports and all that"},				            //14
	{NULL,NULL},							    //15
	{NULL,NULL},							    //16
	{NULL,NULL},								//17
	{NULL,NULL},				                //18
...
static void rec_chat(unsigned short channel,char *text)
{
    ...
    else if (channel==1037) {	// whostaff request
		int cnID;

		cnID=atoi(text);
                do_whotutor(cnID);
    }
    else if (channel==1038) {	// send values
		int cnID,coID;

		cnID=atoi(text);
		coID=atoi(text+11);
		look_values_bg(coID,cnID);
    }
}
...
//Dusan added: flag parameter to indicate /joinall or /join (flag=0)
void join_chat(int cn,int nr, int flag)
{
...
{
	int bit,xID,col;           //there use to be an 'n' declared
	...
	if ((ch[cn].flags&CF_PLAYER) && (channel==0 || channel==32 || channel==15 || channel==16) && !(ch[cn].flags&CF_GOD)) {
	...
	if ((swearing(cn,text)) && (channel != 18)) return 1;

	if (ch[cn].flags&CF_PLAYER) dlog(cn,0,"chat(%s): \"%s\"",cname[channel].name,text);

	//strcpy(name,ch[cn].name);
    
    //You can change the chat color for gods/staffers/tutors here
    int godCOLOR = 0;
    int staffCOLOR = 11;
    int tutorCOLOR = 12;
	int donorCOLOR = 2;
	...
	switch(channel) {
		...
		case 14:	col=8; break;	// sports
		case 30:	col=14; break; // tutor
		case 31:	col=7; break;	// staff
		case 32:	col=5; break;	// god
		default:	col=2; break;
	}
	
	
    //Dusan: Add administrator tags to player name; e.g. "Name<col> says: 
	if (ch[cn].flags&CF_STAFF) {
		sprintf(name,"°c%d°c17%s°c18 [%s]°c%d",staffCOLOR, ch[cn].name,ch[cn].staff_code,col);
	} else if (ch[cn].flags&CF_TUTOR) {
		sprintf(name,"°c%d°c17%s°c18°c%d",tutorCOLOR,ch[cn].name,col);
	} else if (ch[cn].flags&CF_GOD) {
		sprintf(name,"°c%d°c17%s°c18°c%d",godCOLOR,ch[cn].name,col);
	} else if (ch[cn].flags&CF_DONOR) {
		sprintf(name,"°c%d°c17%s°c18°c%d", donorCOLOR, ch[cn].name,col);
	} else sprintf(name,"°c17%s°c18",ch[cn].name);

	if (ch[cn].flags&(CF_STAFF|CF_GOD)) xID=0;
	else xID=ch[cn].ID;
    
    //Dusan added Announcement channelt work
    if (channel==0 && ch[cn].flags&CF_GOD) {
        sprintf(buf,"°c3%s", text);
    ...
    //Emote in chat
    if (((strncmp(text,"/me",3)==0) || (strncmp(text,"/emote",6)==0)) && (channel!=6)){
        if(strncmp(text,"/me",3)==0) text += 3;  //increase pointer by to get rid of '/me' in message
        else    text += 6;                      //else get rid of '/emote'
        //emote chat:
        if(channel==7){
            sprintf(buf,"%010u:%02u:°c%d%s: %s°c%d %s.",
                    xID,
                    get_char_clan(cn),
                    col,
                    cname[channel].name,
            ...
            sprintf(buf,"%010u:%02u:°c%d%s: %s %s.",
                xID,
                get_char_club(cn),
                col,cname[channel].name,
                name,
                text);			// club internal
        }
        else{
    ...
    //Regular chat (not /emote)
    if (channel==7 || channel==12) {        
        if (ch[cn].clan_rank==4) {
            if(ch[cn].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
               sprintf(buf,"%010u:%02u:°c%d%s:°c%d%s(%d)°c%d says: \"%s\"",
                    xID,
                    get_char_clan(cn),
                    col,
                    cname[channel].name,
            ...
            if(ch[cn].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
               sprintf(buf,"%010u:%02u:°c%d%s:%s(%d)°c%d says: \"%s\"",
                    xID,
                    get_char_clan(cn),
                    col,
                    cname[channel].name,
        ...
        if(ch[n].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
           sprintf(buf,"%010u:%02u:°c%d%s:%ssays: \"%s\"",
                xID,
                get_char_club(cn),
                col,
				cname[channel].name,
        ...
        sprintf(buf,"%010u:%02u:°c%d%s: %s says: \"%s\"",
                xID,
                get_char_club(cn),
                col,
				cname[channel].name,
        ...
        if(ch[cn].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
         ...
         if(ch[cn].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
            sprintf(buf,"%010u:%02u:%02u:°c%d%s:%ssays: \"%s\"",
				xID,
				areaID,
				areaM,
				col,
			...
			sprintf(buf,"%010u:%02u:%02u:°c%d%s: %s says: \"%s\"",
				xID,
				areaID,
				areaM,
				col,
        ...
        if(ch[cn].flags&(CF_STAFF|CF_GOD|CF_TUTOR|CF_DONOR))
            sprintf(buf,"%010u°c%d%s:%ssays: \"%s\"",
            xID,
            col,
            cname[channel].name,
            name,		
            ...
            sprintf(buf,"%010u°c%d%s: %s says: \"%s\"",
            xID,
            col,
            cname[channel].name,
            name,		

...
int send_values(int cn,char *name)
{
	int coID;
	char buf[80];
	...
	if (coID==-1) { log_char(cn,LOG_SYSTEM,0,"No player by that name."); return 1; }
    log_char(cn,LOG_SYSTEM,0,"Reminder: Continously spamming /showvalues will result in punishment.");
...
void cmd_tunnel(int cn, char *ptr){
    struct tunnel_ppd *ppd2;        //Structure taken from tunnel.c
    struct tunnel_ppd
    {
        int clevel;			        // current level (resets with every entry)
        unsigned char used[204];	// already used teleport out X times
    };
    ppd2=set_data(cn,DRD_TUNNEL_PPD,sizeof(struct tunnel_ppd));
    int col;    //color for chat; green=2, red=5
    
    //Parse the command to see if /tunnel was used and to find 'level' asked for
    int level;                           //level of tunnel
    while (isspace(*ptr)) ptr++;
        
	level=atoi(ptr);                        //converting ptr into level int.
 
    if((level < 10)||(level > 190)){        //if player asks for out of bounds level
        if(ch[cn].level <= 100){            //check if he's below 100
            if(ch[cn].level<20)  
                level = 10;                 //show default level 10 tunnel for players under 20
            else
                level = ch[cn].level - 10;      //show him by default tunnel -10 his level 
            //getting color for display
            if(ppd2->used[level] < 5)  col = 3;    //red if not used 5 times
            else    col = 2;                       //green if used 5 times
                
            log_char(cn,LOG_SYSTEM,0,"Tunnel %d used:°c%d %d times.", level, col, ppd2->used[level]);
			log_char(cn,LOG_SYSTEM,0,"Tunnel level between %d - %d isnt capped.",ch[cn].level-15,ch[cn].level-5);
        }
        else{           //otherwise if player is above level 100, tunnels stack
            int count = 90;
            //find tunnel not used 10 times, starting at tunnel level 90 and going up (not over -10lvl)
            for(count = 90; (count <= (ch[cn].level - 10)) && (ppd2->used[count] >= 5); count++){}
                if(ppd2->used[count] < 5)  col = 3;    //red if not used 5 times
                else    col = 2;    //green if used 5 times
                log_char(cn,LOG_SYSTEM,0,"Tunnel %d used:°c%d %d times.", count, col, ppd2->used[count]);
				log_char(cn,LOG_SYSTEM,0,"Tunnel level between %d - %d isnt capped.",ch[cn].level-15,ch[cn].level-5);
            }
        }
        else{
...
static int cmd_seyan(int cn)	
{
	...
	if (ch[cn].exp<=100){
		...
		log_char(cn,LOG_SYSTEM,0,"Sorry, you have already gained experience.");
		return 0;
	}

	
	log_char(cn,LOG_SYSTEM,0,"Update scheduled, set Seyan'du.");

    return 1;
}

void cmd_solve_mission(int cn) {
	int nr;
	...
	if (!(ppd->took_mission))
	{
		log_char(cn,LOG_SYSTEM,0,"You don't have a mission.");
		return;
	}
	if (ppd->solved_mission) {
		log_char(cn,LOG_SYSTEM,0,"You have already solved your mission.");
		return;
	}
	
	nr=ppd->took_mission-1;
		
	log_char(cn,LOG_SYSTEM,0,"You solved your mission. Talk to the governor to claim your reward. Mission exp: %d, Mission mil exp: %d (pre-boost/cap)", ppd->mis[nr].exp, ppd->mis[nr].pts);
	ppd->solved_mission=1;
}

void cmd_show_mission(int cn) {
	int nr;
	struct military_ppd *ppd;
	...
	if (!(ppd->took_mission))
	{
		log_char(cn,LOG_SYSTEM,0,"You have °c3not taken°c0 any missions.");
		return;
	}
    //Completed mission
	if (ppd->solved_mission) {
		log_char(cn,LOG_SYSTEM,0,"You have °c2completed°c0 your mission.");
		return;
	}
    nr = ppd->took_mission-1;
    switch(ppd->mis[nr].type) {
		case 1:		log_char(cn,LOG_SYSTEM,0,"Your mission is to °c12slay %d level %d demons°c0 in the Pentagram Quest.",
				    ppd->mis[nr].opt1,
				    ppd->mis[nr].opt2);				
				break;	

		case 3:		log_char(cn,LOG_SYSTEM,0,"Your mission is to °c12find %d units°c0 of silver in the Mine.",
				    ppd->mis[nr].opt1);
				break;
	}		
}

//Dusan added: cmd_giveitem
static void cmd_giveitem(int cn,char *ptr){

    char name[80];
	int co,n, in;
    if (!(in=ch[cn].citem)) {
			log_char(cn,LOG_SYSTEM,0,"You can't give an item that is not in your cursor");
			return;
	}
    ...
    if (!can_carry(co,in,1)){ log_char(cn,LOG_SYSTEM,0,"%s can't carry that.",name); return;}
    if (ch[co].flags&CF_PLAYER) {
        if (!give_char_item(co,in)) { log_char(cn,LOG_SYSTEM,0,"Can't give %s the item. They probably have a full inventory.",name); return;}
        } else {
            if (ch[co].citem) return;
        ...
        if (ch[cn].flags&CF_PLAYER) {
            ...
            log_char(cn,LOG_SYSTEM,0,"You gave a %s (%s°c0) to %s.",it[in].name,it[in].description,ch[co].name);
        }
        if (ch[co].flags&CF_PLAYER) {
            dlog(co,in,"was given %s from %s",it[in].name,ch[cn].name);
            log_char(co,LOG_SYSTEM,0,"°c5A %s (%s°c5) has fallen from the sky. Given by %s.",it[in].name,it[in].description,ch[cn].name);
        }
    
}

//Dusan added: cmd_playerlevel
int cmd_setplevel(int cn,char *ptr)
{
    int co,level,n;
	char buf[80];
    ...
    //Following get's values for 'co' (other charID) and 'level'
	if (isdigit(*ptr) || !*ptr) {
		co=cn;
		level=atoi(ptr);
	} else {
		...
		if (!co) {
			...
			log_char(cn,LOG_SYSTEM,0,"°c5Sorry, no one by the name %s around.",buf);
			return 1;
		}
    ...
    int n2,in;
    //Sets player 'co' level
    ch[co].exp=level2exp(level);
	ch[co].level=level;

	if (level<30) {
		ch[co].flags&=~CF_ARCH;
		ch[co].value[1][V_DURATION]=0;
		ch[co].value[1][V_RAGE]=0;
	}
	if (level>35) {
		ch[co].flags|=CF_ARCH;
		if ((ch[co].flags&(CF_MAGE|CF_WARRIOR))==CF_MAGE && ch[co].value[1][V_DURATION]==0) ch[co].value[1][V_DURATION]=1;
		if ((ch[co].flags&(CF_MAGE|CF_WARRIOR))==CF_WARRIOR && ch[co].value[1][V_RAGE]==0) ch[co].value[1][V_RAGE]=1;
	}
	destroy_chareffects(co);
	for (n2=12; n2<30; n2++) {
		if ((in=ch[co].item[n2])) {
	       destroy_item(in);
	       ch[co].item[n2]=0;
    ...
    log_char(cn,LOG_SYSTEM,0,"°c5Done! %s is now level %d", ch[co].name, level);
    return 1;
}

//Dusan added: cmd_settitle, change value for title
void cmd_settitle(int cn, char *ptr)
{
	struct lostcon_ppd *lppd;
	lppd=set_data(cn,DRD_LOSTCON_PPD,sizeof(struct lostcon_ppd));
    int rank = get_army_rank_int(cn);
    int nr, n;
    int t_flag = 0; //0 if didn't change, 1 if changed.
    int quest_flag = 1;
    ...
    if ((nr < 1) || (nr > 92)){
        log_char(cn,LOG_SYSTEM,0,"You must choose a title between 1 and 91 (see title list on forum).");
        log_char(cn,LOG_SYSTEM,0,"Note: Use /title to toggle title on and off.");
        return;
    }
    else{  //This block does the actual setting and verification for requirements
        //Gods get all titles
        if(ch[cn].flags&CF_GOD){    lppd->titlevalue = nr; t_flag = 1; }
        
        //if it's a fun title (1 - 29)  anyone can use
        if((nr >= 1) && (nr <= 29)){    lppd->titlevalue = nr; t_flag = 1; }
        //if it's a professional title check requirements
        switch(nr) {
                case 30: 	if (ch[cn].prof[0]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 31:	if (ch[cn].prof[1]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 32:	if (ch[cn].prof[2]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 33:	if (ch[cn].prof[3]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 34:	if (ch[cn].prof[4]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 35:	if ((ch[cn].prof[5]) || (ch[cn].prof[6] || (ch[cn].prof[9]))) { 
                                lppd->titlevalue = nr; t_flag = 1;
                            } break;
                case 36:	if (ch[cn].prof[7]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 37:	if (ch[cn].prof[8]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 38:	if (ch[cn].prof[10]) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 39:	if (ch[cn].value[1][V_PROFESSION] >= 30) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 40:	if (ch[cn].value[1][V_PROFESSION] >= 30) { lppd->titlevalue = nr; t_flag = 1;  } break;
        
                //military rank titles
                case 41: 	if (rank >= 1) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 42:	if (rank >= 2) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 43:	if (rank >= 3) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 44:	if (rank >= 4) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 45:	if (rank >= 5) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 46:	if (rank >= 6) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 47:	if (rank >= 7) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 48:	if (rank >= 8) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 49:	if (rank >= 9) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 50:	if (rank >= 10) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 51:	if (rank >= 11) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 52:	if (rank >= 12) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 53:	if (rank >= 13) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 54:	if (rank >= 14) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 55:	if (rank >= 15) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 56:	if (rank >= 16) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 57:	if (rank >= 17) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 58:	if (rank >= 18) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 59:	if (rank >= 19) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 60:	if (rank >= 20) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 61:	if (rank >= 21) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 62:	if (rank >= 22) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 63:	if (rank >= 23) { lppd->titlevalue = nr; t_flag = 1;  } break;
                case 64:	if (rank >= 24) { lppd->titlevalue = nr; t_flag = 1;  } break;
                //Reborn Team titles
                case 65:    if(ch[cn].flags&CF_WON) { lppd->titlevalue = nr; t_flag = 1;  } break;   //Sir and Lady
                case 66:    if(ch[cn].flags&CF_TUTOR)  { lppd->titlevalue = nr; t_flag = 1;  } break;//Tutor title
                case 67:    if (ch[cn].flags&CF_STAFF)   { lppd->titlevalue = nr; t_flag = 1;  } break;//Staff title
                case 68:    if(ch[cn].flags&CF_GOD)   { lppd->titlevalue = nr; t_flag = 1;  } break; //God title
                //achievement titles
                case 69:    if(ch[cn].deaths == 0)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Immortal
                case 70:    if(ch[cn].deaths >= 1)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Zombie
                case 71:    if(ch[cn].deaths >= 10)  { lppd->titlevalue = nr; t_flag = 1;  } break; //RIP
                case 72:    if(ch[cn].deaths >= 50)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Phantom
                case 73:    if(ch[cn].donor_till > 0)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Moneybags
                case 74:    if(ch[cn].clan_rank == 4)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Kingpin
                case 75:    if((questlog_isdone(cn, 5)) && (questlog_isdone(cn, 22))
                               && (questlog_isdone(cn, 39))) { lppd->titlevalue = nr; t_flag = 1;  } break; //Hunter
                case 76:    if(questlog_isdone(cn, 12)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Undead
                case 77:    if(questlog_isdone(cn, 16)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Yummy
                case 78:    if(questlog_isdone(cn, 29)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Snitch
                case 79:    if(questlog_isdone(cn, 28)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Gangster
                case 80:    if(questlog_isdone(cn, 20)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Dragonborn
                case 81:    if(questlog_isdone(cn, 59)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Invader
                case 82:    if(questlog_isdone(cn, 60)) { lppd->titlevalue = nr; t_flag = 1;  } break; //Amazing
                //Case 83 checks that all the quests are done. If a quest is not done, quest_flag set to 0
                case 83:    for(n = 0; ((n < 79) && (quest_flag == 1)) ; n++){
                                if(!(questlog_isdone(cn, n))) { quest_flag = 0;  }
                            }
                            if(quest_flag == 1) { lppd->titlevalue = nr; t_flag = 1; }  break;    //Legendary
                case 84:    if((stats_online_time(cn)/60) >= 24) { lppd->titlevalue = nr; t_flag = 1;  } break; //Novice (1day)
                case 85:    if((stats_online_time(cn)/60) >= 720) { lppd->titlevalue = nr; t_flag = 1;  } break; //Master (1m)
                case 86:    if((stats_online_time(cn)/60) >= 2160) { lppd->titlevalue = nr; t_flag = 1;  } break; //Elder (3m)
                case 87:    if((stats_online_time(cn)/60) >= 3600) { lppd->titlevalue = nr; t_flag = 1;  } break; //Ancient (5m)
                case 88:    if(ch[cn].level >= 50)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Rugged
                case 89:    if(ch[cn].level >= 100)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Hero
                case 90:    if(ch[cn].level >= 200)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Allstar
                case 91:    if(ch[cn].flags&CF_HARDCORE)  { lppd->titlevalue = nr; t_flag = 1;  } break; //Insane
        }
    }
	if(t_flag == 0) log_char(cn,LOG_SYSTEM,0,"Sorry, you do not meet the requirements for the title.");
	reset_name(cn);
}

//Dusan added: cmd_title
void cmd_title(int cn)
{
	ch[cn].flags ^= CF_TITLE;
	log_char(cn,LOG_SYSTEM,0,"Title turned %s.",ch[cn].flags&CF_TITLE ? "°c2On" : "°c3Off");
	reset_name(cn);
}

//Dusan Added: for no hints
void cmd_hints(int cn)
{
	...
	log_char(cn,LOG_SYSTEM,0,"Hints turned %s.",lppd->hints ? "°c3Off" : "°c2On");
}

void cmd_milexp(int cn,char *ptr)

{
	...
	if (val) {
        give_military_pts_no_npc(co,val,1);
		log_char(cn,LOG_SYSTEM,0,"°c5Gave %s %d military exp.",ch[co].name,val);	

	} else {
		log_char(cn,LOG_SYSTEM,0,"%s has %d military exp.",ch[co].name,ch[co].exp);
...
static int cmd_flag(int cn,char *ptr,unsigned long long flag)
{
    ...
    switch(flag) {
        ...
        case CF_MALE:		fptr="male"; break;
		case CF_FEMALE:		fptr="female"; break;
		case CF_DONOR:		fptr="donor"; break;

		default:		fptr="???"; break;
	}

	if (flag==CF_DONOR) {
		if (ch[co].donor_till<time_now){
			ch[co].donor_till=time_now+(60*60*24*30);
			ch[co].flags^=flag;
		} else {
			ch[co].donor_till+=(60*60*24*30);
		}
	} else {
		ch[co].flags^=flag;
	}	

        log_char(cn,LOG_SYSTEM,0,"Set %s %s to %s.",ch[co].name,fptr,(ch[co].flags&flag) ? "°c2On" : "°c3Off");
...
static void cmd_arch(int cn,char *ptr)
{
    char name[80];
	int co,n;
    ...
    //if arch is set on and you're switching it off
    if(ch[co].flags&CF_ARCH){
        //reset the values to 50
        for (n=0; n<=V_IMMUNITY; n++) {
            if (ch[co].value[1][n]>50) {
                ch[co].value[1][n]=50;
            }
...
static void cmd_reset(int cn,char *ptr)
{
    ...
    ch[co].value[1][34]=0; //"empty" skill
...
static void cmd_staffhelp(int cn)
{
    log_char(cn,LOG_SYSTEM,0,"°c5Staff Commands:");
    log_char(cn,LOG_SYSTEM,0,"/exterminate <name>");
    log_char(cn,LOG_SYSTEM,0,"/jump <name> <mirror>");
    log_char(cn,LOG_SYSTEM,0,"/jumplist - display all jump locations");
    ...
    log_char(cn,LOG_SYSTEM,0,"/whot and /whostaff");

}

static void cmd_godhelp(int cn)
{
    log_char(cn,LOG_SYSTEM,0,"°c5God Commands:");
    log_char(cn,LOG_SYSTEM,0,"/arch <name> - toggle on/off");
    log_char(cn,LOG_SYSTEM,0,"/change <name> <sprite#> - change a player's sprites.");
    log_char(cn,LOG_SYSTEM,0,"/chatcolorval - more just a testing thing for me really");
    log_char(cn,LOG_SYSTEM,0,"/create <name>");
	log_char(cn,LOG_SYSTEM,0,"/deleteinvent - deletes inventory only");
    log_char(cn,LOG_SYSTEM,0,"/deneg <player>");
    log_char(cn,LOG_SYSTEM,0,"/exp [name] [amount]");
    log_char(cn,LOG_SYSTEM,0,"/genderswap <player>");
    log_char(cn,LOG_SYSTEM,0,"/ggold <amount>");
    log_char(cn,LOG_SYSTEM,0,"/giveitem <player>");
    ...
    log_char(cn,LOG_SYSTEM,0,"/itemsprite - change sprite of item on cursor");
    log_char(cn,LOG_SYSTEM,0,"/killclan <#> - kills a clan.");
    log_char(cn,LOG_SYSTEM,0,"/killspell <player> - Gods only, removes effects (freeze/warcry).");
    log_char(cn,LOG_SYSTEM,0,"/milexp <name> <val>.");
    log_char(cn,LOG_SYSTEM,0,"/playerlevel <level> - set a players level");
    log_char(cn,LOG_SYSTEM,0,"/qmaster <name>");
    log_char(cn,LOG_SYSTEM,0,"/saves <amount>");
    log_char(cn,LOG_SYSTEM,0,"/setseyan <player>");
    log_char(cn,LOG_SYSTEM,0,"/solve - solves current military mission");
    log_char(cn,LOG_SYSTEM,0,"/speed");
    log_char(cn,LOG_SYSTEM,0,"/staff <name>");
    log_char(cn,LOG_SYSTEM,0,"/staffcode <name> <code>");
    log_char(cn,LOG_SYSTEM,0,"/summon <name>");
    log_char(cn,LOG_SYSTEM,0,"/superman <name> (gods only)");
    log_char(cn,LOG_SYSTEM,0,"/tutor <name>");
    log_char(cn,LOG_SYSTEM,0,"/unlockname  <player>");
    log_char(cn,LOG_SYSTEM,0,"/unpunish  <player>");
}


static void cmd_help(int cn)
{
    log_char(cn,LOG_SYSTEM,0,"°c5Commands:");
    log_char(cn,LOG_SYSTEM,0,"/afk - Sets title to 'AFK' (/title needs to be turned on)");
    ...
    log_char(cn,LOG_SYSTEM,0,"/color - displays current color selection");
    ...
    log_char(cn,LOG_SYSTEM,0,"/hints, to turn on and off hints");
    ...
    log_char(cn,LOG_SYSTEM,0,"/joinall channels");
    ...
    log_char(cn,LOG_SYSTEM,0,"/mission - display your current mission status");
    log_char(cn,LOG_SYSTEM,0,"/murmur <text> - like say, but only within close range");
	log_char(cn,LOG_SYSTEM,0,"/mirrors - shows a list of extra mirrors.");
    log_char(cn,LOG_SYSTEM,0,"/noexp - Do not receive xp. toggle on/off.");
    ...
    log_char(cn,LOG_SYSTEM,0,"/rules - view the rules for the server");
    ...
    log_char(cn,LOG_SYSTEM,0,"/settitle <#> - # value of title you want, leave blank to see options");
    log_char(cn,LOG_SYSTEM,0,"/shout <text> - like say, but with increased range");
    log_char(cn,LOG_SYSTEM,0,"/showvalues <player> - allows players to send values.");
    log_char(cn,LOG_SYSTEM,0,"/sort - sort the contents of your inventory");
    log_char(cn, LOG_SYSTEM,0,"/spawn - displays the timers for clan spawners");
    ...
    log_char(cn,LOG_SYSTEM,0,"/title - Toggle Sir/Lady title.");
    log_char(cn,LOG_SYSTEM,0,"/tunnel and /tunnel <level>");
...
//Dusan Added
static void cmd_ruleslist(int cn)
{

    log_char(cn,LOG_SYSTEM,0,"°c3Server Rules, Conduct and Player Expectations:");
    log_char(cn,LOG_SYSTEM,0,"1. Do °c3NOT°c0 insult, bully, or abuse players.");
    log_char(cn,LOG_SYSTEM,0,"2. Swearing is °c2okay! °c0Just °c3don't°c0 over do it.");
    log_char(cn,LOG_SYSTEM,0,"3. Acct. sharing is °c2allowed°c0 (exc. tutors/staff). Losses °c3won't°c0 be refunded.");
    log_char(cn,LOG_SYSTEM,0,"4. Do °c3not°c0 sell or advertise selling your account.");
    log_char(cn,LOG_SYSTEM,0,"5. Cheating of any kind (macro, bug abuse, etc.) will °c3not°c0 be tolerated.");
    log_char(cn,LOG_SYSTEM,0,"6. °c3No°c0 impersonating tutors/staffers/gods.");
    log_char(cn,LOG_SYSTEM,0,"7. °c2Enjoy your stay!°c0");

    log_char(cn,LOG_SYSTEM,0,"°c3This is NOT a comprehensive list and may change at any time.");
    log_char(cn,LOG_SYSTEM,0,"°c3Please use common sense before acting.");
    log_char(cn,LOG_SYSTEM,0,"°c3We aim to be understanding and fair. If you wish to discuss a punishment, contact game@astoniareborn3.com.");
    log_char(cn,LOG_SYSTEM,0,"°c3Remember no one ever got punished for being nice. Just be cool.");

}
    
//Dusan Added
static void cmd_jumplist(int cn)
{
    log_char(cn,LOG_SYSTEM,0,"°c5List of All /Jump Names:");
	log_char(cn,LOG_SYSTEM,0,"aston - 167,188,3");
    log_char(cn,LOG_SYSTEM,0,"rd - 250,250,14");
	log_char(cn,LOG_SYSTEM,0,"elysium - 12,178,3");
	log_char(cn,LOG_SYSTEM,0,"fort - 126,179,1");
	log_char(cn,LOG_SYSTEM,0,"zomb1 - 5,5,2");
	log_char(cn,LOG_SYSTEM,0,"zomb2 - 3,86,2");
	log_char(cn,LOG_SYSTEM,0,"skel2 - 85,85,1");
	log_char(cn,LOG_SYSTEM,0,"skel3 - 184,226,1");
	log_char(cn,LOG_SYSTEM,0,"mages - 154,106,1");
	log_char(cn,LOG_SYSTEM,0,"knights - 163,82,1");
	log_char(cn,LOG_SYSTEM,0,"trans - 130,201,3");
	log_char(cn,LOG_SYSTEM,0,"mine - 231,242,12");
	log_char(cn,LOG_SYSTEM,0,"hole - 236,176,3");
	log_char(cn,LOG_SYSTEM,0,"lq - 245,245,20");
	log_char(cn,LOG_SYSTEM,0,"bran - 203,227,29");
	log_char(cn,LOG_SYSTEM,0,"hole2 - 226,164,29");
	log_char(cn,LOG_SYSTEM,0,"smuggle - 103,107,26");
	log_char(cn,LOG_SYSTEM,0,"grim - 210,247,31");
	log_char(cn,LOG_SYSTEM,0,"exkor - 67,108,17");
	log_char(cn,LOG_SYSTEM,0,"job - 228,228,32");
	log_char(cn,LOG_SYSTEM,0,"tunnel - 250,250,33");
	log_char(cn,LOG_SYSTEM,0,"teufel - 250,250,34");
    log_char(cn,LOG_SYSTEM,0,"°c5Jumps to teleporters:");
    log_char(cn,LOG_SYSTEM,0,"cam - 139,75,1");		 
	log_char(cn,LOG_SYSTEM,0,"aston2 - 129,201,3");	 
	log_char(cn,LOG_SYSTEM,0,"eu1 - 239,249,6");	         
	log_char(cn,LOG_SYSTEM,0,"eu2 - 92,164,6");	         
	log_char(cn,LOG_SYSTEM,0,"eu3 - 49,135,6");	         
	log_char(cn,LOG_SYSTEM,0,"eu4 - 14,114,6");	         
	log_char(cn,LOG_SYSTEM,0,"eu5 - 5,4,6");			     
	log_char(cn,LOG_SYSTEM,0,"eu6 - 172,36,6");		     
	log_char(cn,LOG_SYSTEM,0,"eu7 - 225,249,34");			 
	log_char(cn,LOG_SYSTEM,0,"iu1 - 93,102,10");			 
	log_char(cn,LOG_SYSTEM,0,"iu2 - 11,113,10");			 
	log_char(cn,LOG_SYSTEM,0,"iu3 - 241,87,10");			 
	log_char(cn,LOG_SYSTEM,0,"iu4 - 213,156,11");			 
	log_char(cn,LOG_SYSTEM,0,"iu5 - 189,80,11");	
	log_char(cn,LOG_SYSTEM,0,"nomad - 16,124,19");		 
	log_char(cn,LOG_SYSTEM,0,"forest - 181,117,16");		 
	log_char(cn,LOG_SYSTEM,0,"cali - 230,62,36");			 
	log_char(cn,LOG_SYSTEM,0,"ark - 28,20,37");
}
...
void show_lostconppd(int cn,struct lostcon_ppd *ppd)
{
	...
	if (ch[cn].value[1][V_FLASH]) log_char(cn,LOG_SYSTEM,0,"Don't use Ball Lightning [/NOBALL]: %s.",ppd->noball ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_BLESS]) log_char(cn,LOG_SYSTEM,0,"Don't use Bless [/NOBLESS]: %s.",ppd->nobless ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_FIREBALL]) log_char(cn,LOG_SYSTEM,0,"Don't use Fireball [/NOFIREBALL]: %s.",ppd->nofireball ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_FLASH]) log_char(cn,LOG_SYSTEM,0,"Don't use Lightning Flash [/NOFLASH]: %s.",ppd->noflash ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_FREEZE]) log_char(cn,LOG_SYSTEM,0,"Don't use Freeze [/NOFREEZE]: %s.",ppd->nofreeze ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_HEAL]) log_char(cn,LOG_SYSTEM,0,"Don't use Heal [/NOHEAL]: %s.",ppd->noheal ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_MAGICSHIELD]) log_char(cn,LOG_SYSTEM,0,"Don't use Magic Shield [/NOSHIELD]: %s.",ppd->noshield ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_PULSE]) log_char(cn,LOG_SYSTEM,0,"Don't use Pulse [/NOPULSE]: %s.",ppd->nopulse ? "°c2On" : "°c3Off");
	if (ch[cn].value[1][V_WARCRY]) log_char(cn,LOG_SYSTEM,0,"Don't use Warcry [/NOWARCRY]: %s.",ppd->nowarcry ? "°c2On" : "°c3Off");

	log_char(cn,LOG_SYSTEM,0,"Don't use Healing Potions [/NOLIFE]: %s.",ppd->nolife ? "°c2On" : "°c3Off");
	log_char(cn,LOG_SYSTEM,0,"Don't use Mana Potions [/NOMANA]: %s.",ppd->nomana ? "°c2On" : "°c3Off");
	log_char(cn,LOG_SYSTEM,0,"Don't use Combo Potions [/NOCOMBO]: %s.",ppd->nocombo ? "°c2On" : "°c3Off");
	log_char(cn,LOG_SYSTEM,0,"Don't use Recall Scroll [/NORECALL]: %s.",ppd->norecall ? "°c2On" : "°c3Off");
	log_char(cn,LOG_SYSTEM,0,"Don't Move [/NOMOVE]: %s.",ppd->nomove ? "°c2On" : "°c3Off");

	log_char(cn,LOG_SYSTEM,0,"Automation Settings:");
	if (ch[cn].value[1][V_BLESS]) log_char(cn,LOG_SYSTEM,0,"Automatic Re-Bless [/AUTOBLESS]: %s.",ppd->autobless ? "°c2On" : "°c3Off");	
	if (ch[cn].value[1][V_PULSE]) log_char(cn,LOG_SYSTEM,0,"Automatic Pulse [/AUTOPULSE]: %s.",ppd->autopulse ? "°c2On" : "°c3Off");	
	log_char(cn,LOG_SYSTEM,0,"Automatic Turning [/AUTOTURN]: %s.",ppd->autoturn ? "°c2On" : "°c3Off");

	log_char(cn,LOG_SYSTEM,0,"Protection Settings:");
	log_char(cn,LOG_SYSTEM,0,"Allow others to bless me [/ALLOWBLESS]: %s.",(ch[cn].flags&CF_NOBLESS) ? "°c3No" : "°c2Yes");
}
...
int command(int cn,char *ptr)	// 1=ok, 0=repeat
{
	...
	if ((len=cmdcmp(ptr,"immortal",2)) && is_lqmaster(cn)) {
		...
		log_char(cn,LOG_SYSTEM,0,"Immortal is %s.",(ch[cn].flags&CF_IMMORTAL) ? "°c2On" : "°c3Off");
	...
	if ((len=cmdcmp(ptr,"infrared",3)) && is_lqmaster(cn)) {
		...
		log_char(cn,LOG_SYSTEM,0,"Infrared is %s.",(ch[cn].flags&CF_INFRARED) ? "°c2On" : "°c3Off");
	...
	if ((len=cmdcmp(ptr,"playerkiller",12))) {
        if (ch[cn].flags&CF_PK) {
            ...
            if 
                (ch[cn].level<10) log_char(cn,LOG_SYSTEM,0,"Sorry, you may not become a player killer before reaching level 10.");
			else
                log_char(cn,LOG_SYSTEM,0,
                 "°c3Please take a moment to consider this decision. If another player kills you, he will be able to take all your belongings, or kill you over and over again. Do you really want this? Type: '/iwilldie %d' to confirm.",ch[cn].ID);
		}

		log_char(cn,LOG_SYSTEM,0,"PK is %s.",(ch[cn].flags&CF_PK) ? "°c2On" : "°c3Off");
	...
	if ((len=cmdcmp(ptr,"iwilldie",8))) {
		...
		log_char(cn,LOG_SYSTEM,0,"Invisible is %s.",(ch[cn].flags&CF_INVISIBLE) ? "°c2On" : "°c3Off");
    ...
    //Dusan added: removed Staffers from seeing stats
	if ((len=cmdcmp(ptr,"values",6)) && (ch[cn].flags&(CF_GOD))) {
                ptr+=len;
    ...
    if ((len=cmdcmp(ptr,"showvalues", 10))) {
                ptr+=len;
		
		while (isspace(*ptr)) ptr++;

		return send_values(cn,ptr);
	}
	...
	if ((len=cmdcmp(ptr,"create",3)) && (ch[cn].flags&CF_GOD)) {
		...
		if (!(ch[cn].flags&CF_LAG) && in_arena(cn)) {
			...
			ch[cn].flags^=CF_LAG;
			log_char(cn,LOG_SYSTEM,0,"Turned artificial lag %s.",(ch[cn].flags&CF_LAG) ? "°c2On" : "°c3Off");
	...
	if ((len=cmdcmp(ptr,"thief",3))) {
		ch[cn].flags^=CF_THIEFMODE;
                log_char(cn,LOG_SYSTEM,0,"Turned thief mode %s.",(ch[cn].flags&CF_THIEFMODE) ? "°c2On" : "°c3Off");
		update_char(cn);
                return 1;
	}
	...
	if ((len=cmdcmp(ptr,"notells",3))) {
		ch[cn].flags^=CF_NOTELL;
                log_char(cn,LOG_SYSTEM,0,"Turned no-tell mode %s.",(ch[cn].flags&CF_NOTELL) ? "°c2On" : "°c3Off");
		update_char(cn);
                return 1;
	}
    ...
    if ((len=cmdcmp(ptr,"arch",4)) && (ch[cn].flags&(CF_GOD))) {
		ptr+=len; while (isspace(*ptr)) ptr++;
	...
	if (cmdcmp(ptr,"status",0)) {
		...
		log_char(cn,LOG_SYSTEM,0,"°c2Account Status:");
		log_char(cn,LOG_SYSTEM,0,"Your account will not expire. The game is free to play!");
		log_char(cn,LOG_SYSTEM,0,"You own %d payment tokens.",ch[cn].payment_tokens);
		if(ch[cn].flags&CF_DONOR){
			int t = (ch[cn].donor_till)-time_now;
			if(t>0){
				log_char(cn,LOG_SYSTEM,0,"°c2Your donor status will last for another %d days, %d hours, and %d minutes.",t/(60*60*24),(t/(60*60))%24,(t/60)%60);
			}
		}
    ...
    if ((len=cmdcmp(ptr,"solve",5)) && (ch[cn].flags&CF_GOD)) {
		cmd_solve_mission(cn);
		return 1;
	}

    if ((len=cmdcmp(ptr,"mission",7))) {
		cmd_show_mission(cn);
		return 1;
	}

    if (cmdcmp(ptr,"noexp",5)){
        ch[cn].flags^=CF_NOEXP;
		log_char(cn,LOG_SYSTEM,0,"Turned noexp mode %s.",(ch[cn].flags&CF_NOEXP) ? "°c2On" : "°c3Off");
		update_char(cn);
		return 1;
    }

    if ((cmdcmp(ptr,"staffhelp",9)  && (ch[cn].flags&CF_GOD))){
        cmd_staffhelp(cn);
        return 1;
    }

    if ((cmdcmp(ptr,"godhelp",7)  && (ch[cn].flags&CF_GOD))){
        cmd_godhelp(cn);
        return 1;
    }
    
    //Dusan: Command '/color', see current color code (fixed to show 3 values instead of total)
    if (cmdcmp(ptr,"color",5)) {
        
        int col1_red,col1_green,col1_blue;
        int col2_red,col2_green,col2_blue;
        int col3_red,col3_green,col3_blue;
        
        //converting bitwise color totals into individual RGB color values
        col1_red = ((ch[cn].c1&64512)>>10);
        col2_red = ((ch[cn].c2&64512)>>10);
        col3_red = ((ch[cn].c3&64512)>>10);
        
        col1_green = (((ch[cn].c1)&1023)>>5);
        col2_green = (((ch[cn].c2)&1023)>>5); 
        col3_green = (((ch[cn].c3)&1023)>>5);
        
        col1_blue = (((ch[cn].c1)&1023)&31);
        col2_blue = (((ch[cn].c2)&1023)&31);
        col3_blue = (((ch[cn].c3)&1023)&31);  
        
        //Display results
		log_char(cn,LOG_SYSTEM,0,"Your current colors are: °c2c1°c0 = %d %d %d, °c2c2°c0 = %d %d %d, °c2c3°c0 = %d %d %d",
                 col1_red, col1_green, col1_blue, 
                 col2_red, col2_green, col2_blue,
                 col3_red, col3_green, col3_blue);

		return 1;
	}
    //Dusan: Command for testing purposes... just want to see all the colors
    if ((len=cmdcmp(ptr,"chatcolorval",11)) && (ch[cn].flags&(CF_GOD))) {
        int n;
        for(n = 0; n < 20; n++){
            log_char(cn,LOG_SYSTEM,0,"°c%dThis is col%d", n,n);
        }
        return 1;
    }
    
    //Command:  /joinall
    if ((len=cmdcmp(ptr,"joinall", 5))) {
		int nr;
        for (nr= 0; nr<=32; nr++) {
		  join_chat(cn,nr, 1);
        }
        log_char(cn,LOG_SYSTEM,0,"Joined all channels.");

        return 1;
	}
    if ((len=cmdcmp(ptr,"jumplist",5)) && (ch[cn].flags&(CF_STAFF|CF_GOD))) {
        cmd_jumplist(cn);
        return 1;
    }

    //Command: allows gods to give items to players
    if ((len=cmdcmp(ptr,"giveitem",8)) && (ch[cn].flags&(CF_GOD))) {
		ptr+=len; while (isspace(*ptr)) ptr++;
		cmd_giveitem(cn,ptr);
		return 1;
	}
    //Command: displays the server rules
    if ((len=cmdcmp(ptr,"rules",4))) {
		cmd_ruleslist(cn);
		return 1;
	}
    //Dusan Command: set title to 'AFK'
    if ((len=cmdcmp(ptr,"afk",3))) {
		struct lostcon_ppd *lppd;
	    lppd=set_data(cn,DRD_LOSTCON_PPD,sizeof(struct lostcon_ppd));
        lppd->titlevalue = 999;
        reset_name(cn);    
    ...
    //Command: /spawn
    if ((len=cmdcmp(ptr,"spawns",5))) {
		cmd_cjtimer(cn);
        return 1;
	}
    
    //Command: /playerlevel <player> <level>
    if ((len=cmdcmp(ptr,"playerlevel",11)) && (ch[cn].flags&(CF_GOD))) {
        ptr+=len;
        cmd_setplevel(cn, ptr);
		return 1;
	}
    
    //Command: /deneg <player>
    if((len=cmdcmp(ptr,"deneg",3)) && (ch[cn].flags&CF_GOD)) {
		ptr+=len; while (isspace(*ptr)) ptr++;
		cmd_deneg(cn,ptr);
		return 1;
	}

    //Dusan added: title to toggle title on/off
    if ((len=cmdcmp(ptr,"title",5))) {
		cmd_title(cn);
		return 1;
	}
	
    //Dusan added: title to toggle title on/off
    if ((len=cmdcmp(ptr,"settitle",8))) {
        ptr+=len; while (isspace(*ptr)) ptr++;
		cmd_settitle(cn, ptr);
		return 1;
    ...
    //Dusan added Command: /tunnel <level>, see how many times tunnel was used.
    if ((len=cmdcmp(ptr,"tunnel",6))) {
		ptr+=len;
        cmd_tunnel(cn, ptr);
        return 1;
	}

    //Command: /killspell <name> removes effects
    if ((len=cmdcmp(ptr,"killspell",9)) && (ch[cn].flags&(CF_GOD))) {
		int n,co,pos,val,in;
    ...
    //Commands: /hints turn them on or of
	if (cmdcmp(ptr,"hints",4)){
		cmd_hints(cn);
		return 1;
	}
	...
	if ((len=cmdcmp(ptr,"tokens",5)) && (ch[cn].flags&CF_GOD)) {
		int tokens;
		...
		ch[cn].payment_tokens=tokens;
		ch[cn].flags|=CF_ITEMS;

		return 1;
	}
	if ((len=cmdcmp(ptr,"claim",5))) {
        int qty,tokens,cnt,cnt2;
		...
		if (qty<1) {
			log_char(cn,LOG_SYSTEM,0,"You do not have any tokens to claim.");
			return 1;
		}
		cnt=ch[cn].payment_tokens;
		cnt2=qty;
		tokens=cnt2+cnt;
		ch[cn].payment_tokens=tokens;
		setTokens(cn,0);
		log_char(cn,LOG_SYSTEM,0,"Received %d Tokens.",qty);

        return 1;
	}
	if ((len=cmdcmp(ptr,"csprite",7)) && (ch[cn].flags&(CF_GOD|CF_DONOR))) {
		int spr;
        ...
        //Demon sprites used for Teufel access not allowed
        if(spr==27 || spr==157 || spr==39){
            log_char(cn,LOG_SYSTEM,0,"Sprite %d is disabled for use.", spr); 
			return 1;
        }
        //(disallow negatives, 0, 56, 120, 128-133, 172, 175, 191-199, 358, 359, 362-399, 427-499, 548, 549) Ends at 577 + giant spider at 1000
		if (spr<=0 || spr==56 || spr==120 || (spr>=128 && spr<=133) || spr==172 || spr==175 ||(spr>=191 && spr<=199) || spr==358 || spr==359 || (spr>=362 && spr<=399) || (spr>=427 && spr<=499) || spr==548 || spr==549 || (spr>=578 && spr<=999) || spr>=1001) {
			log_char(cn,LOG_SYSTEM,0,"Cant go any higher, all invisible sprites.."); 
			return 1;
		}
	...
	if (cmdcmp(ptr,"killexppot",7)) {
		int n,in;
		for (n=12; n<30; n++) {
			if ((in=ch[cn].item[n]) && it[in].driver==IDR_EXPBONUS) {
				destroy_effect_type(cn,EF_POTION);
				destroy_item(in);
				ch[cn].item[n]=0;
				update_char(cn);
				log_char(cn,LOG_SYSTEM,0,"Exp potion removed.");
				return 1;
			} else if ((in=ch[cn].item[n]) && it[in].driver==IDR_TOKENEXPBONUS) {
				destroy_effect_type(cn,EF_POTION);
				destroy_item(in);
				ch[cn].item[n]=0;
				update_char(cn);
		...
		log_char(cn,LOG_SYSTEM,0,"No experience potion found.");
                return 1;
	}
	 ...
	 if ((len=cmdcmp(ptr,"special",7)) && (ch[cn].flags&CF_GOD)) {
		...
		if ((in=create_special_item(20,1,1,10000))) {
			if (ch[cn].flags&CF_PLAYER) dlog(cn,in,"took from #create");
			ch[cn].citem=in;
			ch[cn].flags|=CF_ITEMS;
			it[in].carried=cn;
    ...
    if ((len=cmdcmp(ptr,"deleteinvent",12)) && (ch[cn].flags&(CF_GOD))) {
        int n,in;
        ...
        update_char(cn);
        log_char(cn,LOG_SYSTEM,0,"°c3Deleted Inventory!");
        return 1;
    }
	if (cmdcmp(ptr,"mirrors",6)) {
		log_char(cn,LOG_SYSTEM,0,"°c3List of currently running areas that have extra mirror's.");
		log_char(cn,LOG_SYSTEM,0,"Cameron: 1 - 13 - 20.");
		log_char(cn,LOG_SYSTEM,0,"Earth Pents: 1 - 9 - 18.");
		log_char(cn,LOG_SYSTEM,0,"Fire Pents: 1 - 13.");
		log_char(cn,LOG_SYSTEM,0,"Ice Pents: 1 - 13.");
		log_char(cn,LOG_SYSTEM,0,"Random Dungeons: 1 - 13.");
		return 1;
	}

		...
		if (msg->type==NT_TEXT) {
			analyse_text_driver(cn,msg->dat1,(char*)msg->dat2,msg->dat3);
				...
				if (ppd && (ptr=strcasestr((char*)msg->dat2,"deposit"))) {
					...
					val=atoi(result)*100;
                    int nr_val = atoi(result);
                    if (val) {
                        if(nr_val > 999999){
                            quiet_say(cn,"Thou canst not deposit more than 999,999 gold per transaction.");
                        }
                        else if(val < 1){
                            quiet_say(cn,"Thou must say a positive amount.");
                        }
						else if (val>ch[co].gold || val<1) {
					...
					if (val) {
                        if(nr_val > 999999){
                            quiet_say(cn,"Thou canst not withdraw more than 999,999 gold per transaction.");
                        }
                        else if(val < 1){
                            quiet_say(cn,"Thou must say a positive amount.");
                        }else if (val>ppd->imperial_gold || val<1) {

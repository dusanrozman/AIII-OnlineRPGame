...
struct qa qa[]={
	...
	{{"clan",NULL},"If you wish to found a clan then tell me the clan number you want, the name you want that clan to have, and hand me a Clan Jewel. Use: 'clan: <nr> <clan name>'. If you wish to be Clan #1 and want to name your clan 'Black Rose', use: 'clan: 1 Black Rose'. Be aware that the game will use the phrase 'The <clan name> clan', ie. 'The Black Rose Clan', so avoid 'The' and 'Clan' in the name.",0},
	{{"jewels",NULL},NULL,2},
	{{"repeat",NULL},NULL,3},
		...
		if (msg->type==NT_TEXT) {
			analyse_text_driver(cn,msg->dat1,(char*)msg->dat2,msg->dat3);
                        ...
                        if ((msg->dat1==1 || msg->dat1==2) && (co=msg->dat3)!=cn) {	// talk, and not our talk				
                                if ((ptr=strcasestr((char*)msg->dat2,"clan:")) && (fnd=set_data(co,DRD_CLANFOUND,sizeof(struct clan_found_data)))) {
					if (!get_char_clan(co) && !get_char_club(co)) {
						ptr+=5;
						while (isspace(*ptr)) ptr++;
                        //Dusan adding choose clan# - 'clan: # clan_name'
                        char clan_num[2];
                        int clan_num_as_int;
                        n = 0;
                        while((isspace(*ptr)) || (n <2)){
                            clan_num[n] = *ptr;    //getting the number
                            n++; ptr++;
                        }
                        clan_num_as_int = atoi(clan_num);   //turning char num into int num
						for (n=0; n<79; n++) {
							if (!*ptr || *ptr=='"') break;
							fnd->name[n]=*ptr++;
						}
						fnd->name[n]=0;
                        fnd->nr = clan_num_as_int;  //save number in clan list
						fnd->state=1;
						quiet_say(cn,"Your clan, %s, will be named '%s'. It will be Clan %d.Try again if that is not what you want. Or hand me a Clan Jewel to proceed. You can buy them at Jeremy's",ch[co].name,fnd->name, fnd->nr);
			...
			co=msg->dat1;
				...
				if (it[in].ID==IID_CLANJEWEL && (fnd=set_data(co,DRD_CLANFOUND,sizeof(struct clan_found_data)))) {
						...
						if (!res) {
                            ...
                            quiet_say(cn,"Please choose a clan number between 1 and 31.");
                        }
                        else if(res == -3){
                            quiet_say(cn,"The clan number you requested has already been taken.");
                        }
                        else {
							quiet_say(cn,"There was an error creating your clan. Please try again.");							
						}
					} else {
						quiet_say(cn,"You must name and number of your clan first. Say: 'clan: <#> <clan-name>'. E.g, 'clan: 1 Black Rose'");
					}
				}
				// try to give it back
				dat->give_try++;
		...
		if (msg->type==NT_TEXT) {
			...
			if ((msg->dat1==1 || msg->dat1==2) && (co=msg->dat3)!=cn && char_see_char(cn,co)) {	// talk, and not our talk
                if ((ptr=strcasestr((char*)msg->dat2,"deposit"))) {
                    ...
                    //Dusan bug checking (will only allow 6 digiti numbers)
                    ptr+=8; //gets rid 'deposit ' (including space)
                    char result[6];
                    for(n = 0; (n < 6) || (ptr==NULL); n++)
                    {
                        result[n] = *ptr;
                        ptr++;
                    }
					...
					if (nr<1) {
						...
						continue;
							...
							say(cn,"Oi, stop that! Too much gold.");
							remove_message(cn,msg);
							continue;
				    }
                    int val;
				...
				if ((ptr=strcasestr((char*)msg->dat2,"withdraw"))) {
                    ...
                    //Dusan bug checking (will only allow 6 digiti numbers)
                    ptr+=9; //gets rid 'withdraw ' (including space)
                    char result[6];
                    for(n = 0; (n < 6) || (ptr==NULL); n++)
                    {
                        result[n] = *ptr;
					...
					nr=atoi(result);   //convert to an int
...
{
	int diff,freq,cnr,stmr,cj;
	char *cname,buf[256],msg[256];
    ...
    if (!cn) {	// timer calls
		...
		if (!*(unsigned int*)(it[in].drdata+4)) {
			...
			*(unsigned int*)(it[in].drdata+4)=(realtime+(RANDOM(60*60*freq/2)+60*60*freq/4));
			it[in].max_level=it[in].drdata[0];
			stmr=(*(int*)(it[in].drdata+4)-realtime);
			stmr+=time_now;
			sprintf(buf,"update merc1.spawners set time=%d,stack=0 where max=%d",stmr,it[in].drdata[0]);
			if (mysql_query(&mysql,buf)) elog("Could not save new server reset spawn time for %d spawn.",it[in].drdata[0]);
		}
		
		int tg = (*(unsigned int*)(it[in].drdata+4) - realtime)/(60);
		// announce at certain minutes
		if (tg == 10 || tg == 5 || tg == 1) {

			if (!(tg == 1)) {
				sprintf(msg,"0000000000°c15Spawner Announcement: %d spawner %d to go.",it[in].max_level,tg);
				server_chat(5,msg);
			}
		}
        if (realtime>=*(unsigned int*)(it[in].drdata+4)) {
			...
			*(unsigned int*)(it[in].drdata+4)=(realtime+(RANDOM(60*60*freq)+60*60*freq/2));
			stmr=(*(int*)(it[in].drdata+4)-realtime);
			stmr+=time_now;
			
			sprintf(buf,"update merc1.spawners set time=%d,stack=%d where max=%d",stmr,it[in].drdata[2],it[in].drdata[0]);
			if (mysql_query(&mysql,buf)) elog("Could not save new spawn time for %d spawn.",it[in].drdata[0]);
		}
		return;
	}
	...
	if (cname && cnr) {
		...
		sprintf(buf,"update merc1.spawners set stack=%d where max=%d",(it[in].drdata[2]-1),it[in].drdata[0]);
		if (mysql_query(&mysql,buf)) elog("Could not save last grabber for spawn %d.",it[in].max_level);
        cj=create_item("clan_jewel");
		sprintf(buf,"0000000000°c15Clan Announcement: %s won a Jewel for %s from level %d!",ch[cn].name,cname,it[in].drdata[0]);
		server_chat(5,buf);
        if (!give_char_item(cn,cj)) destroy_item(cj);
	} else {
		sprintf(buf,"0000000000°c15Clan Announcement: %s has won the spawn at level %d!  What will %s do with it?",ch[cn].name,it[in].max_level,hename(cn));
		server_chat(5,buf);
